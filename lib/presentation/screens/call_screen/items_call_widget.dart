  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: InkWell(
        ...
        child: ListTile(
          leading: model.image!.length<20?Image.asset(model.image!,width: 50,height: 50,):Image.network(model.image!),
          title: Row(
            children: [
              Text(model.name,style: AppStyles.style16.copyWith(color: Colors.white),overflow: TextOverflow.ellipsis,),
              const Spacer(),
              Text(subStringForDate(date: model.dateTime),style: AppStyles.style15.copyWith(color: Colors.white),)
          ...
          subtitle: Row(
            children: [
              Text(model.status??''),
              const Spacer(),
              model.status=='called'?const Icon(Icons.call_made_outlined,color: Colors.green,):const Icon(Icons.phone_callback,color: Colors.red,)
            ],
          ),
